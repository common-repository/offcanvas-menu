 ...
 */
	namespace Enweby\Offcanvas\Vendor\Boo\AdminSettingsHelper;
...
//		    TODO: if the option to store file as file url
		...
		/**
		 * Displays a select box for creating the menu select box
		 *
		 * @param array $args settings field args
		 */
		function callback_menus( $args ) {
			//Getting available menu terms.
			$nav_taxonomy= get_terms([
							'taxonomy' => 'nav_menu',
							'hide_empty' => false,
						]);
			...
			foreach($nav_taxonomy as $nav)
			{
				$options [ $nav->slug ] = esc_html($nav->name);
			}
			
			//update option values in args.
			$args['options'] = $options;

			//$menu_args = wp_parse_args( $nav_args, $default_args );
			$menu_args = $args;
			$this->callback_select( $menu_args );
		}
